...
export class Camera {
  constructor(gl, cameraType, node) {
    this.gl = gl;
    ...
    if (cameraType == undefined) {
      ...
      glMatrix.mat4.perspective(
        projMatrix,
        node.props.perspective.yfov,
        node.props.perspective.aspectRatio,
        node.props.perspective.znear,
        node.props.perspective.zfar
      );
    } else if (cameraType === "orthographic") {
      //ortho(out, left, right, bottom, top, near, far)
      glMatrix.mat4.ortho(
        projMatrix,
        -node.props.orthographic.xmag,
        node.props.orthographic.xmag,
        -node.props.orthographic.ymag,
        node.props.orthographic.ymag,
        node.props.orthographic.znear,
        node.props.orthographic.zfar
      );
    }
  ...
  onkeypress(e) {
    if (e.key === "w") {
      this.translate_eye("W");
    }
    if (e.key === "s") {
      this.translate_eye("S");
    }
    if (e.key === "a") {
      this.translate_eye("A");
    }
    if (e.key === "d") {
      this.translate_eye("D");
    }
  } //press
