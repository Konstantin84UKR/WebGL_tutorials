...
//--fetch
export async function LoadShaderTextUsingFetch(URL) {
...
export async function createFetchShaderProgram(gl, URL_vs, URL_fs){

    let vertex_shader_fetch = await LoadShaderTextUsingFetch(URL_vs);
    let fragment_shader_fetch = await LoadShaderTextUsingFetch(URL_fs);

    let vShader = getShader(gl, 'vs', vertex_shader_fetch); if (!vShader) return null;
    let fShader = getShader(gl, 'fs', fragment_shader_fetch); if (!fShader) return null;
